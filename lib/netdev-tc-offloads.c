                   ...
                   struct dpif_flow_stats *stats OVS_UNUSED)
{
    ...
    if (handle && prio) {
        VLOG_DBG_RL(&rl, "updating old chain: %d, prio: %d, handle: %d",
                    chain, prio, handle);
        del_filter_and_ufid_mapping(ifindex, chain, prio, handle, block_id, ufid);
    }
                   ...
                   struct ofpbuf *buf)
{
    ...
    VLOG_DBG_RL(&rl, "flow get (dev %s chain %d prio %d handle %d block_id %d)",
                netdev_get_name(dev), chain, prio, handle, block_id);
                   ...
                   struct dpif_flow_stats *stats)
{
    ...
    error = del_filter_and_ufid_mapping(ifindex, chain, prio, handle, block_id, ufid);
