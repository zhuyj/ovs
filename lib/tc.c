        ...
        if (nl_attr_type(nla) == TCA_PEDIT_KEY_EX) {
            ...
            for (int j = 0; j < ARRAY_SIZE(flower_pedit_map); j++) {
                ...
                if ((keys->off >= mf && keys->off < mf + sz)
                    ...
                    uint32_t *dst = (uint32_t *) (rewrite_key + diff);
                    uint32_t *dst_m = (uint32_t *) (rewrite_mask + diff);
             ...
             uint32_t **mask, uint32_t **data)
{
    ...
    char *rewrite_key = (void *) &flower->rewrite.key;
    char *rewrite_mask = (void *) &flower->rewrite.key;
    ...
    *data = (uint32_t *) (rewrite_key + src_offset - diff);
    *mask = (uint32_t *) (rewrite_mask + src_offset - diff);
}

static inline
void csum_update_flag_for_proto(struct tc_flower *flower) {
