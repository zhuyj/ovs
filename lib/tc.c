...
nl_parse_flower_ip(struct nlattr **attrs, struct tc_flower *flower) {
    ...
    if (attrs[TCA_FLOWER_KEY_FLAGS_MASK]) {
        uint32_t flags = ntohl(nl_attr_get_u32(attrs[TCA_FLOWER_KEY_FLAGS]));
        uint32_t flags_mask =
            ntohl(nl_attr_get_u32(attrs[TCA_FLOWER_KEY_FLAGS_MASK]));

        key->ip_frag = flags & TCA_FLOWER_KEY_FLAGS_IS_FRAGMENT;
        mask->ip_frag = flags_mask & TCA_FLOWER_KEY_FLAGS_IS_FRAGMENT;
    }
...
nl_msg_put_flower_options(struct ofpbuf *request, struct tc_flower *flower)
{
    ...
    if (host_eth_type == ETH_P_IP || host_eth_type == ETH_P_IPV6) {
        ...
        if (flower->mask.ip_frag) {
            flags_mask |= TCA_FLOWER_KEY_FLAGS_IS_FRAGMENT;

            if (flower->key.ip_frag) {
                flags |= TCA_FLOWER_KEY_FLAGS_IS_FRAGMENT;
            }
        }

        if (flags_mask) {
            nl_msg_put_u32(request, TCA_FLOWER_KEY_FLAGS, htonl(flags));
