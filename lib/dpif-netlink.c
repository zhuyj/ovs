...
parse_flow_put(struct dpif_netlink *dpif, struct dpif_flow_put *put)
{
    ...
    size_t offset = nl_msg_start_nested(&buf, OVS_FLOW_ATTR_ACTIONS);
    struct nlattr *act;
    struct netdev *dev;
    int err;

    parse_key_and_mask_to_match(put->key, put->key_len, put->mask,
                                put->mask_len, &match);
    ...
    NL_ATTR_FOR_EACH (nla, left, put->actions, put->actions_len) {
        if (nl_attr_type(nla) == OVS_ACTION_ATTR_OUTPUT) {
            ...
            odp_port_t out_port =  nl_attr_get_u32(nla);
    ...
    VLOG_DBG("failed offloading flow: %s", ovs_strerror(err));
...
dpif_netlink_operate(struct dpif *dpif_, struct dpif_op **ops, size_t n_ops)
{
    ...
    if (!netdev_flow_api_enabled) {
        ...
        new_ops = xmalloc(sizeof (struct dpif_op *) * n_ops);
        n_new_ops = 0;
